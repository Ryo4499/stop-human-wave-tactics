    ...
    return (
      ...
      >
        ...
        {article.seo?.metaSocial?.map((metaSocial) => {
          return (
            <Head key={metaSocial?.id}>
              <MetaSocial
                sns={String(metaSocial?.socialNetwork)}
                title={String(metaSocial?.title)}
                description={String(metaSocial?.description)}
                image={String(metaSocial?.image?.data?.attributes?.previewUrl)}
              />
            </Head>
          );
        })}
        <Head>
          <meta
            name="robots"
            content={String(article.seo?.metaRobots)}
            key="robots"
          />
          <meta
            name="keywords"
            content={String(article.seo?.keywords)}
            key="keywords"
          />
        </Head>
        <Grid>
